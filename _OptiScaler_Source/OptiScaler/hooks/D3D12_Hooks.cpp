        ...
        if (restoreComputeSignature)
        {
            ...
            if (extendedRestoreSignature)
            {
                ...
                if (s_SetComputeRootConstantBufferView.o_lateHook != nullptr)
                    DetourAttach(&(PVOID&) s_SetComputeRootConstantBufferView.o_lateHook, hkSetComputeRootConstantBufferViewLate);

                if (s_SetComputeRootShaderResourceView.o_lateHook != nullptr)
                    DetourAttach(&(PVOID&) s_SetComputeRootShaderResourceView.o_lateHook, hkSetComputeRootShaderResourceViewLate);

                if (s_SetComputeRootUnorderedAccessView.o_lateHook != nullptr)
                    DetourAttach(&(PVOID&) s_SetComputeRootUnorderedAccessView.o_lateHook, hkSetComputeRootUnorderedAccessViewLate);
            }
        }
        ...
        if (restoreGraphicSignature)
        {
            ...
            if (extendedRestoreSignature)
            {
                if (s_SetGraphicsRootDescriptorTable.o_lateHook != nullptr)
                    DetourAttach(&(PVOID&) s_SetGraphicsRootDescriptorTable.o_lateHook, hkSetGraphicsRootDescriptorTableLate);

                if (s_SetGraphicsRoot32BitConstant.o_lateHook != nullptr)
                    DetourAttach(&(PVOID&) s_SetGraphicsRoot32BitConstant.o_lateHook, hkSetGraphicsRoot32BitConstantLate);

                if (s_SetGraphicsRoot32BitConstants.o_lateHook != nullptr)
                    DetourAttach(&(PVOID&) s_SetGraphicsRoot32BitConstants.o_lateHook, hkSetGraphicsRoot32BitConstantsLate);

                if (s_SetGraphicsRootConstantBufferView.o_lateHook != nullptr)
                    DetourAttach(&(PVOID&) s_SetGraphicsRootConstantBufferView.o_lateHook, hkSetGraphicsRootConstantBufferViewLate);

                if (s_SetGraphicsRootShaderResourceView.o_lateHook != nullptr)
                    DetourAttach(&(PVOID&) s_SetGraphicsRootShaderResourceView.o_lateHook, hkSetGraphicsRootShaderResourceViewLate);

                if (s_SetGraphicsRootUnorderedAccessView.o_lateHook != nullptr)
                    DetourAttach(&(PVOID&) s_SetGraphicsRootUnorderedAccessView.o_lateHook, hkSetGraphicsRootUnorderedAccessViewLate);
            }
        }
                                        ...
                                        IID_PPV_ARGS(&commandList)) == S_OK)
        {
                ...
                s_SetComputeRootShaderResourceView.o_earlyHook || s_SetComputeRootUnorderedAccessView.o_earlyHook)
            {
                ...
                if (s_SetComputeRootDescriptorTable.o_earlyHook != nullptr && extendedRestoreSignature)
                    DetourAttach(&(PVOID&) s_SetComputeRootDescriptorTable.o_earlyHook, hkSetComputeRootDescriptorTable);
                ...
                if (s_SetComputeRootConstantBufferView.o_earlyHook != nullptr && extendedRestoreSignature)
                    DetourAttach(&(PVOID&) s_SetComputeRootConstantBufferView.o_earlyHook, hkSetComputeRootConstantBufferView);

                if (s_SetComputeRootShaderResourceView.o_earlyHook != nullptr && extendedRestoreSignature)
                    DetourAttach(&(PVOID&) s_SetComputeRootShaderResourceView.o_earlyHook, hkSetComputeRootShaderResourceView);

                if (s_SetComputeRootUnorderedAccessView.o_earlyHook != nullptr && extendedRestoreSignature)
                    DetourAttach(&(PVOID&) s_SetComputeRootUnorderedAccessView.o_earlyHook, hkSetComputeRootUnorderedAccessView);
...
{
    LOG_DEBUG("Adapter: {:X}, Level: {:X}, Caller: {}", (size_t) pAdapter, (UINT) MinimumFeatureLevel, Util::WhoIsTheCaller(_ReturnAddress()));
    ...
    if (result == S_OK && ppDevice != nullptr && MinimumFeatureLevel != D3D_FEATURE_LEVEL_1_0_CORE && !nonPrimaryGpu)
    {
        ...
        if (State::Instance().currentD3D12Device->QueryInterface(IID_PPV_ARGS(&infoQueue)) == S_OK)
        {
            ...
            if (infoQueue->QueryInterface(IID_PPV_ARGS(&infoQueue1)) == S_OK && infoQueue1 != nullptr)
            {
                ...
                res = infoQueue1->RegisterMessageCallback(D3D12DebugCallback, D3D12_MESSAGE_CALLBACK_FLAG_NONE, NULL, NULL);
            }
        }
#endif
    }
...
static HRESULT hkCreateDevice(ID3D12DeviceFactory* pFactory, IUnknown* pAdapter, D3D_FEATURE_LEVEL MinimumFeatureLevel, REFIID riid, void** ppDevice)
{
    LOG_DEBUG("Adapter: {:X}, Level: {:X}, Caller: {}", (size_t) pAdapter, (UINT) MinimumFeatureLevel, Util::WhoIsTheCaller(_ReturnAddress()));
